...
#include "LRingChart.h"
#include "qpainter.h"
#include "qpainterpath.h"
#include "qevent.h"
#include "qdebug.h"
#include<QRectF>
LRingChart::LRingChart(QWidget *parent) : QWidget(parent)
{
    showTitle = true;
    titleHeight = 18;
    titleFontSize = 10;
    ...
    setOutPieInfos("單血氧,60|CCHD,30|大字體界面,10");
    setInPieInfos("常規界面,40|ECG界面,30|波形界面,30");
...
void LRingChart::mouseMoveEvent(QMouseEvent *event)
{
    ...
    for (int i = 0; i < outPieInfo.size(); i++) {
        if (outPieInfo.at(i).path.contains(event->pos())) {
            outPieInfo[i].offset = hoverOffset;
            contains = true;
        } else {
...
void LRingChart::paintEvent(QPaintEvent *)
{
    //绘制准备工作,启用反锯齿
...
void LRingChart::drawBg(QPainter *painter)
{
    painter->save();
...
void LRingChart::drawOutCircle(QPainter *painter)
{
    int radius = 90;
    painter->save();
    painter->setPen(Qt::NoPen);
...
void LRingChart::drawMidCircle(QPainter *painter)
{
    int radius = 50;
    painter->save();
    painter->setPen(Qt::NoPen);
...
void LRingChart::drawInCircle(QPainter *painter)
{
    int radius = 10;
    painter->save();
    painter->setPen(Qt::NoPen);
...
void LRingChart::drawOutPie(QPainter *painter, qreal scale, QPoint center)
{
    int radius = 75;
    int startAngle = 0;
    painter->save();
    ...
    //根据用户数据绘制圆
    for (int i = 0; i < outPieInfo.size(); i++) {
        int offset = outPieInfo.at(i).offset;
        int angle = outPieInfo.at(i).percent * 16 * 3.6;
        QColor color = outPieInfo.at(i).color;
        painter->setPen(Qt::NoPen);

        QPainterPath path;
        int pathInfactRaius = radius;
        ...
        if (offset > 0) {
            QPainterPath path;
            int pathInfactRaius = radius + offset;
            int pathScaleRadius = scale * pathInfactRaius;
            path.arcTo(-pathScaleRadius, -pathScaleRadius, 2 * pathScaleRadius, 2 * pathScaleRadius, startAngle / 16, angle / 16);
            path.translate(center);
...
void LRingChart::drawInPie(QPainter *painter, qreal scale, QPoint center)
{
    int radius = 35;
    int startAngle = 0;
    painter->save();
    ...
    //根据用户数据绘制圆
    for (int i = 0; i < inPieInfo.size(); i++) {
        int offset = inPieInfo.at(i).offset;
        int angle = inPieInfo.at(i).percent * 16 * 3.6;
        QColor color = inPieInfo.at(i).color;
        painter->setPen(Qt::NoPen);
...
void LRingChart::drawTitle(QPainter *painter)
{
    painter->save();
...
void LRingChart::drawLegendText(QPainter *painter, qreal scale)
{
    int rectWidth = 12;
    int rectRadius = 2;
    painter->save();
    ...
    //绘制左侧图例区域文字
    int fontHegiht = painter->fontMetrics().height() + 3;
    int textHeight = fontHegiht * scale;
    int textPosy = qRound((this->height() - textHeight) * 1.0 / scale) - 3;
    for (int i = 0; i < outPieInfo.size(); i++) {
        painter->save();
        painter->setPen(Qt::NoPen);
    ...
    textPosy = qRound((this->height() - textHeight) * 1.0 / scale) - 3;
    int textPosx = qRound(this->width() * 1.0 / scale - maxwidth - 60);
    for (int i = 0; i < inPieInfo.size(); i++) {
        painter->save();
        painter->setPen(Qt::NoPen);
...
bool LRingChart::getShowTitle() const
{
    return this->showTitle;
}

int LRingChart::getTitleHeight() const
{
    return this->titleHeight;
}

int LRingChart::getTitleFontSize() const
{
    return this->titleFontSize;
}

QString LRingChart::getTitle() const
{
    return this->title;
}

bool LRingChart::getShowLegend() const
{
    return this->showLegend;
}

int LRingChart::getLegendHeight() const
{
    return this->legendHeight;
}

int LRingChart::getLegendFontSize() const
{
    return this->legendFontSize;
}

int LRingChart::getHoverOffset() const
{
    return this->hoverOffset;
}

int LRingChart::getHoverAlpha() const
{
    return this->hoverAlpha;
}

QColor LRingChart::getBgColor() const
{
    return this->bgColor;
}

QColor LRingChart::getTextColor() const
{
    return this->textColor;
}

QColor LRingChart::getHighColor() const
{
    return this->highColor;
}

QColor LRingChart::getFlagColor() const
{
    return this->flagColor;
}

QColor LRingChart::getOutCircleColor() const
{
    return this->outCircleColor;
}

QColor LRingChart::getMidCircleColor() const
{
    return this->midCircleColor;
}

QColor LRingChart::getInCircleColor() const
{
    return this->inCircleColor;
}

QString LRingChart::getOutPieInfos() const
{
    return this->outPieInfos;
}

QString LRingChart::getInPieInfos() const
{
    return this->inPieInfos;
}

QSize LRingChart::sizeHint() const
{
    return QSize(400, 300);
}

QSize LRingChart::minimumSizeHint() const
{
    return QSize(40, 30);
}

void LRingChart::setShowTitle(bool showTitle)
{
...
void LRingChart::setTitleHeight(int titleHeight)
{
...
void LRingChart::setTitleFontSize(int titleFontSize)
{
...
void LRingChart::setTitle(const QString &title)
{
    if (this->title != title) {
        this->title = title;
        this->update();
...
void LRingChart::setShowLegend(bool showLegend)
{
...
void LRingChart::setLegendHeight(int legendHeight)
{
...
void LRingChart::setLegendFontSize(int legendFontSize)
{
    if (this->legendFontSize != legendFontSize) {
        this->legendFontSize = legendFontSize;
        this->update();
    }
}

void LRingChart::setHoverOffset(int hoverOffset)
{
    if (this->hoverOffset != hoverOffset) {
        this->hoverOffset = hoverOffset;
        this->update();
    }
}

void LRingChart::setHoverAlpha(int hoverAlpha)
{
    if (this->hoverAlpha != hoverAlpha) {
        this->hoverAlpha = hoverAlpha;
        this->update();
    }
}

void LRingChart::setBgColor(const QColor &bgColor)
{
...
void LRingChart::setTextColor(const QColor &textColor)
{
...
void LRingChart::setHighColor(const QColor &highColor)
{
...
void LRingChart::setFlagColor(const QColor &flagColor)
{
...
void LRingChart::setOutCircleColor(const QColor &outCircleColor)
{
...
void LRingChart::setMidCircleColor(const QColor &midCircleColor)
{
...
void LRingChart::setInCircleColor(const QColor &inCircleColor)
{
...
void LRingChart::setOutPieInfos(const QString &outPieInfos)
{
...
void LRingChart::setInPieInfos(const QString &inPieInfos)
{
...
void LRingChart::setOutPieColors(const QList<QColor> &outPieColors)
{
    this->outPieColors = outPieColors;
    this->update();
}

void LRingChart::setInPieColors(const QList<QColor> &inPieColors)
{
    this->inPieColors = inPieColors;
    this->update();
}

void LRingChart::clearOutPie()
{
    outPieInfo.clear();
    this->update();
}

void LRingChart::clearInPie()
{
    inPieInfo.clear();
    this->update();
}

void LRingChart::appendOutPie(const RingData &data)
{
    outPieInfo.append(data);
    this->update();
}

void LRingChart::appendInPie(const RingData &data)
{
    inPieInfo.append(data);
    this->update();
}
